class PaymentsController < ApplicationController
	...
	def checkout
		...
		if @my_payment.nil?
			...
			Stores::Paypal.checkout(params[:PayerID],params[:paymentId]) do 
				...
				@my_payment.pay!
				redirect_to ok_path, notice:"se proceso correctamente el pago"
				return
			end
			redirect_to carrito_path, notice:"Hubo un error"
		end
	end
	...
	def process_card
		...
		if paypal_helper.process_card(params).create
			...
			@my_payment.pay!
			redirect_to carrito_path,notice: "El pago se realizo correctamente con tarjeta "
		else
